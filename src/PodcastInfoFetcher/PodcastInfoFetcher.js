...
const blackedOutBackgroundConstantURL = 'https://github.com/caravana-do-absurdo/pop-out-on-hover-element/blob/main/res/img/$ID_OF_PODCAST_background_hidden.png?raw=true'
const podcastURLs = {
...
const PodcastInfoFetcher = (podcastID) => ({
  ...
  getBlackedOutBackgroundImageURL: () => {
    return blackedOutBackgroundConstantURL.replace('$ID_OF_PODCAST', podcastID.toLowerCase());
  },
  getOnClickURLToRedirect: () => {
    return podcastURLs[getTreatedPodcastID(podcastID)] ? podcastURLs[getTreatedPodcastID(podcastID)] : null
  },
  getReleaseDate: () => {
    return releaseDates[getTreatedPodcastID(podcastID)] ? releaseDates[getTreatedPodcastID(podcastID)] : null
  },
  isPodcastReleased: (today) => {
    let releaseDateOfPodcast = releaseDates[getTreatedPodcastID(podcastID)] ? releaseDates[getTreatedPodcastID(podcastID)] : null
